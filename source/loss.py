...
def AE(observe_x, normal, p_xz: dist.Distribution):
    """
    :param observe_x: (batch_size, x_dims)
    :param observe_z: (sample_size, batch_size, z_dims) or (batch_size, z_dims,)
    :param normal: (batch_size, x_dims)
    ...
    normal = torch.unsqueeze(normal, 0)  # (1, batch_size, x_dims)
    log_p_xz = p_xz.log_prob(observe_x)  # (1, batch_size, x_dims)
    return - torch.mean(torch.sum(log_p_xz * normal, -1))
    

def KL(observe_z, normal, q_zx: dist.Distribution, p_z: dist.Distribution):
    """
    :param observe_z: (sample_size, batch_size, z_dims) or (batch_size, z_dims,)
    :param normal: (batch_size, x_dims)
    :param q_zx: samples in shape (sample_size, batch_size, z_dims)
    ...
    return  - torch.mean(log_p_z*radio - log_q_zx)

def KL0(observe_z, normal, q_zx: dist.Distribution, p_z: dist.Distribution):
    """
    :param observe_z: (sample_size, batch_size, z_dims) or (batch_size, z_dims,)
    :param normal: (batch_size, x_dims)
    :param q_zx: samples in shape (sample_size, batch_size, z_dims)
    ...
    return - torch.mean(log_p_z - log_q_zx)
